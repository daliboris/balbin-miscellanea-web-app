...
declare variable $maintenancePage := "maintenance.html";
declare variable $loginPage := "login.html";

declare variable $allowOrigin := local:allowOriginDynamic(request:get-header("Origin"));
declare variable $loginRequired := true();
declare variable $maintenanceMode := false();
...
(: Maintenance mode - set $maintenanceMode to true to enable maintenance mode :)
if($maintenanceMode) then
    if (matches($exist:resource, "\.(woff|woff2|css|svg|ico)$", "s")) then
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{$exist:controller}/{$exist:path}"/>
    </dispatch>
    else
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{$exist:controller}/templates/{$maintenancePage}">
         <set-header name="Content-Type" value="text/html"/>
        </forward>
    </dispatch>
else
(: Redirect empty path to root path with trailing slash :)
...
(: IIIF image requests are handled by the IIIF image server module; 
can ends with image extension, must be processed first 
:)
else if (matches($exist:path, "^/+api/+iiif/+")) then 
<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
...
else if ($loginRequired and not(sm:is-authenticated()) and 
    (
    matches($exist:path, "\.(html|htm)$") or 
    matches($exist:path, '(noble-houses|people|places|organizations|bibliography)$')
    )
    and not($exist:path eq "/" || $loginPage)) then
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <redirect url="{$loginPage}" />
    </dispatch>

(: if the user is logged in and access the login file he is redirected to default page :)
else if ($loginRequired and sm:is-authenticated() and ($exist:path eq "/" || $loginPage)) then
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <redirect url="{$landingPage}" />
    </dispatch>
